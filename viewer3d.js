...
function createViewer(root){
  ...
  const fallbackMode=(message='La 3D n’est pas disponible sur cet appareil.')=>{
    root.classList.add('v6-viewer-fallback-mode');
    if(loading){
      loading.style.display='grid';
      const b=loading.querySelector('b'),sp=loading.querySelector('span');
      if(b)b.textContent='Mode photo activé';
  ...
  if(!gl){fallbackMode('WebGL 2 n’est pas disponible. Le site reste utilisable en mode photo.');return;}
  ...
  if(mobile&&captureBtn)captureBtn.style.display='none';
  ...
  function go(name){const p=presets[name]||presets.global;if(name==='machine')setStage('works');goalPos.set(...p.p);goalTarget.set(...p.t);cameraAnimating=true;viewButtons.forEach(b=>b.classList.toggle('active',b.dataset.view===name));if(title)title.textContent=p.n;if(copy)copy.textContent=p.c;stopTour(false)}
  ...
  // Stable direct rendering. Heavy SSAO post-processing was removed because it caused
  // context loss / frame drops on some browsers. Lighting, PBR materials and shadows remain.

  function resize(){const rect=root.getBoundingClientRect(),w=Math.max(320,rect.width),h=Math.max(540,rect.height);camera.aspect=w/h;camera.updateProjectionMatrix();renderer.setSize(w,h,false)}
  ...
  renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();fallbackMode('Le navigateur a suspendu le moteur 3D. Le site bascule en mode photo pour rester fluide.');},{passive:false});
  renderer.domElement.addEventListener('webglcontextrestored',()=>{root.classList.remove('v6-viewer-fallback-mode');if(loading)loading.style.display='none'});
...
function boot(el){if(el.dataset.ready)return;el.dataset.ready='1';try{createViewer(el)}catch(err){console.warn('ORIS REALSITE fallback',err);el.classList.add('v6-viewer-fallback-mode');const loading=el.querySelector('.viewer-loading');if(loading){loading.style.display='grid';const b=loading.querySelector('b'),sp=loading.querySelector('span');if(b)b.textContent='Mode photo activé';if(sp)sp.textContent='La 3D n’a pas pu démarrer. Le reste du site fonctionne normalement.';setTimeout(()=>loading.style.display='none',2200)}}}
