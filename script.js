(() => {
  ...
  if(loader){
    const seen=sessionStorage.getItem('orisV6Intro');
    if(seen||reduced){loader.classList.add('done')}
      ...
      const run=now=>{
        ...
        if(t<1)requestAnimationFrame(run); else setTimeout(()=>{loader.classList.add('done');sessionStorage.setItem('orisV6Intro','1')},160);
      };
  ...
  if(viewerWatch.length)setTimeout(()=>viewerWatch.forEach(v=>{if(v.classList.contains('v6-viewer-ready'))return;v.classList.add('v6-viewer-fallback-mode');const l=v.querySelector('.viewer-loading');if(l){l.querySelector('b').textContent='Mode photo activé';l.querySelector('span').textContent='La 3D n’est pas disponible sur cette connexion ou cet appareil.';setTimeout(()=>l.style.display='none',1800)}const s=v.querySelector('.v6-viewer__status span');if(s)s.textContent='REALSITE / MODE PHOTO'}),8000);
...
// V6.1 3D watchdog: prevents an endless loader if the module/CDN/WebGL fails.
(() => {
  const arm = () => {
    document.querySelectorAll('[data-viewer360]').forEach(viewer => {
      setTimeout(() => {
        if (viewer.classList.contains('v6-viewer-ready')) return;
        viewer.classList.add('v6-viewer-fallback-mode');
        ...
        if (loading) {
          ...
          if (title) title.textContent = 'Mode photo activé';
          if (text) text.textContent = 'La 3D n’a pas répondu assez vite. Le site reste entièrement utilisable.';
