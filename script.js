...
(()=>{
 ...
 if(location.pathname.endsWith('devis.html')&&new URLSearchParams(location.search).get('diagnostic')==='1'){
  let diag='';try{diag=sessionStorage.getItem('orisDiagnostic')||''}catch(_){};if(diag){const ta=document.querySelector('textarea[name="description"],textarea[name="project"],textarea');if(ta&&!ta.value.trim()){ta.value='Diagnostic ORIS Express :\n\n'+diag;ta.dispatchEvent(new Event('input',{bubbles:true}))}}
 }
